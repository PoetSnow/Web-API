...
namespace Gemstar.BSPMS.HotelScanOrder.Web.Models.Pay
{
    ...
    /// <summary>
    /// 会员卡余额和增值金额支付，由会员系统自动根据参数来决定扣款顺序和比例
    /// </summary>
    public class PayActionXmlMbrCardAndLargess : PayActionXmlBase
    {

        //首先产生一条待支付的数据，同时返回 接口调用需要的参数。(外部代码以及消费项目明细Id)
            ...
            if (result != null)
            {
                if (result.ErrorNo == "1")
                {
                    ...
                    xmlStrBuilder.Append("<ProfileCa>")
                                 ...
                                 .Append("<BalanceType>").Append(((byte)BalanceType.储值).ToString("D2")).Append(((byte)BalanceType.赠送金额).ToString("D2")).Append("</BalanceType>")
